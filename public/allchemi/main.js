...
function renderGrid(){
  const filtered = activeFilter === 'all'
    ? projects
    : projects.filter(p => activeFilter === 'video' ? (p.media_type === 'video' || p.media_type === 'youtube') : p.media_type === activeFilter);
...
function renderLightbox(){
  ...
  lightboxMedia.innerHTML = p.media_type === 'video'
    ...
    : p.media_type === 'youtube'
    ? `<div style="position:relative; width:100%; aspect-ratio:16/9;"><iframe style="position:absolute; inset:0; width:100%; height:100%;" src="https://www.youtube.com/embed/${p.media_path}?autoplay=1" title="${escapeHtml(p.title)}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe></div>`
