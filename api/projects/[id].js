...
module.exports = async function handler(req, res) {
  ...
  if (req.method === 'DELETE') {
    ...
    try {
      if (existing.media_path) {
        await del(existing.media_path);
      }

      if (existing.thumbnail_path) {
        await del(existing.thumbnail_path);
      }
    } catch (_) {}
  ...
  if (req.method === 'PUT') {
    try {
      ...
      const featured =
        fields.featured?.[0] !== undefined
          ? Number(fields.featured[0])
          : existing.featured;

      const sort_order =
        fields.sort_order?.[0] !== undefined
          ? Number(fields.sort_order[0])
          : existing.sort_order;
